...
def fetch(search_terms: list[str], where: str) -> list[RawJob]:
    ...
    with httpx.Client(timeout=15.0) as client:
        for term in search_terms:
            try:
                resp = client.get(
                    "https://remotive.com/api/remote-jobs", params={"search": term, "limit": 20}
                )
            ...
            for item in data.get("jobs", []):
                try:
                    ext_id = first_id(item.get("id"), item.get("url"))
                    title = (item.get("title") or "").strip()
                    # A listing with no id or no title is unusable noise.
                    if not ext_id or not title or ext_id in seen_ids:
                        continue
                    seen_ids.add(ext_id)
